...
model=oldb
output_dir=oldb_devss0.5
CODEPATH=$HOME/FINAL/
job_name=${model}${prob_num}_tempconv
job_folder=${model}${prob_num}'_devss0.5_tempconv'
...
outpath=output/${output_dir}/${model}${prob_num}/beta${beta_name}/tempconv
...
echo input_arg${COUNTER}=\'-input ${infile} -output output${COUNTER} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} -beta_s ${beta_s}\' >> ${pbs_file}
