...
model=oldb
output_dir=oldb_ex2_fixed
CODEPATH=$HOME/FINAL/
job_name=${model}${prob_num}_batch
job_folder=${output_dir}${prob_num}
...
do
for N in 8 16 #4 8 12 16
do
for K in K1 K4_Kx2Ky2 #K2_Kx1Ky2 K2_Kx2Ky1
do
for L in _L64 #_L8 _L16 _L32 _L64
do
...
echo input_arg${COUNTER}=\'-input ${infile} -output output${COUNTER} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} -cons_it 0\' >> ${pbs_file}
...
echo 'if [ "$(eval pwd)" != "${WDPATH}" ]; then' >> ${pbs_file}
echo 'echo '\''ERROR: Not in correct working directory!!'\' >> ${pbs_file}
echo 'exit' >> ${pbs_file}
echo 'fi' >> ${pbs_file}
