...
cat ../../scripts/oldb_devss1.0/oldb${prob_num}_tempconv_we${we}_K${K}_N${N}.sh >> oldb${prob_num}_tempconv_we${we}_K${K}_N${N}.pbs

###############################################################################################################################################################
# Spatial Convergence.v
###############################################################################################################################################################
t=0.0001
...
cat ../../scripts/oldb_devss1.0/oldb${prob_num}_spatconv_we${we}_dt${t}_K${K}.sh >> oldb${prob_num}_spatconv_we${we}_dt${t}_K${K}.pbs
done
...
echo '#!/bin/bash' > oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -q SMP_queue >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -N oldb${prob_num} >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -o oldb${prob_num}_we${we}_dt${t}_K${K}-out.txt >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -e oldb${prob_num}_we${we}_dt${t}_K${K}-err.txt >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -l select=1:ncpus=16:mpiprocs=1 >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -l place=scatter:excl >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PBS' -l walltime=48:00:00 >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo '#PROJECT=PR58' >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo 'module load compiler/intel-13' >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo 'cd $PBS_O_WORKDIR' >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
echo >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs

cat ../../scripts/oldb_devss1.0/oldb${prob_num}_we${we}_dt${t}_K${K}.sh >> oldb${prob_num}_we${we}_dt${t}_K${K}.pbs
