...
K=20
N=8
outpath=output/oldb_devss0.5/oldb${prob_num}/beta${beta_name}/tempconv/ 
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} -beta_s ${beta_s} '>' ${outfile_screen}' && echo Finished ' Beta=${beta} We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_tempconv_beta${beta_name}_we${we}_K${K}_N${N}.sh
...
###############################################################################################################################################################
t=0.0001
outpath=output/oldb_devss0.5/oldb${prob_num}/beta${beta_name}/spatconv/
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} -beta_s ${beta_s} '>' ${outfile_screen}' && echo Finished ' Beta=${beta} We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_spatconv_beta${beta_name}_we${we}_dt${t}_K${K}.sh
...
do
outpath=output/oldb_devss0.5/oldb${prob_num}/beta${beta_name}/we${we}/
for K in 26 31
do
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} -beta_s ${beta_s} '>' ${outfile_screen}' && echo Finished ' Beta=${beta} We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_beta${beta_name}_we${we}_dt${t}_K${K}.sh
