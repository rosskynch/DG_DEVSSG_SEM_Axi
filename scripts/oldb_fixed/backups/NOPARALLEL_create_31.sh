...
K=20
N=8
outpath=output/oldb_fixed/oldb${prob_num}/tempconv/ 
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen} >> oldb${prob_num}_tempconv_we${we}_K${K}_N${N}.sh

echo 'echo Finished a run' >> oldb${prob_num}_tempconv_we${we}_K${K}_N${N}.sh
...
###############################################################################################################################################################
t=0.0001
outpath=output/oldb_fixed/oldb${prob_num}/spatconv/
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen} >> oldb${prob_num}_spatconv_we${we}_dt${t}_K${K}.sh

echo 'echo Finished a run' >> oldb${prob_num}_spatconv_we${we}_dt${t}_K${K}.sh
...
do
outpath=output/oldb_fixed/oldb${prob_num}/we${we}/
for K in 26 31
do
for N in 14 15 16 17 18
do

echo '# Script for prob'${prob_num} we=${we} dt=${t} K=${K} N=${N} > oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh
echo 'export OMP_NUM_THREADS=1' >> oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh
echo 'cd ../../' >> oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh
echo >> oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh

outfile_screen=${outpath}oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.out
outfile=${outpath}oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}
infile=${inpath}${K}.dat

echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen} >> oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh

echo 'echo Finished a run' >> oldb${prob_num}_we${we}_dt${t}_K${K}_N${N}.sh
