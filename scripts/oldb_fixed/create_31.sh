...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen}' && echo Finished 'We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_tempconv_we${we}_K${K}_N${N}.sh
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen}' && echo Finished 'We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_spatconv_we${we}_dt${t}_K${K}.sh
...
echo ./vesemd -input ${infile} -output ${outfile} -prob ${prob_num} -N ${N} -alphaz 1.0 -deltat ${t} -re ${re} -we ${we} -beta ${beta} '>' ${outfile_screen}' && echo Finished 'We=${we} N=${N} K=${K} Deltat=${t} '&' >> oldb${prob_num}_we${we}_dt${t}_K${K}.sh
